	...
	public boolean generate(World worldIn, BlockPosBeta doorBase, EnumFacing dirForward, Size size, Block doorBlock,
		...
		case LARGE:
			...
			if (tentDim) {
				...
				super.buildLayer(worldIn, doorBase, dirForward, Content.barrier, BP_LARGE.getBarrierCoords());
			}
			return true;
		case MEDIUM:
			// build all relevant layers
			...
			if (tentDim) {
				...
				super.buildLayer(worldIn, doorBase, dirForward, Content.barrier, BP_MED.getBarrierCoords());
			}
			return true;
		case SMALL:
			// build all relevant layers
			...
			if (tentDim) {
				// build barrier
				super.buildLayer(worldIn, doorBase, dirForward, Content.barrier, BP_SMALL.getBarrierCoords());
			}
			return true;
		}
		return false;
	...
	public boolean isValidForFacing(World worldIn, BlockPosBeta doorBase, Size size, EnumFacing facing) {
		...
		Predicate<Block> checkBlockPred = new Predicate<Block>() {
			...
			public boolean test(Block b) {
				return b instanceof ITepeeBlock;
			}
		};

	...
	public boolean onBlockActivated(World worldIn, int x, int y, int z, EntityPlayer player, int side, float hitX,
		...
		if (!worldIn.isRemote) {
			...
			if (te != null && te instanceof TileEntityTentDoor) {
				...
				int dir = TentDimension.isTent(worldIn) ? TentDimension.STRUCTURE_DIR
						: struct.getNewStructure().isValidForFacing(worldIn, new BlockPosBeta(x, baseY, z),
								StructureType.Size.SMALL, facing);
				...
				System.out.println("Error! Failed to retrieve TileEntityYurtDoor at " + x + ", " + baseY + ", " + z);
		}
		return false;
	}
